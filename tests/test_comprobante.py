...
class TestValidacionComprobante:
    ...
    def test_fecha_hora_numerica_lanza_error(self):
        # Convencion de tiempos: la fecha y hora real es un datetime.
    ...
    def test_nro_cero_lanza_error(self):
        with pytest.raises(DatosInvalidos, match="nro"):
            Comprobante(0, _solicitud_basica(), _hora(9, 30), "Juan")

    def test_nro_negativo_lanza_error(self):
        with pytest.raises(DatosInvalidos, match="nro"):
            Comprobante(-1, _solicitud_basica(), _hora(9, 30), "Juan")


class TestConstruccionComprobante:
    def test_guarda_atributos_basicos(self):
        s = _solicitud_basica()
        c = Comprobante(1, s, _hora(9, 30), "Juan")
        assert c.nro == 1
        assert c.solicitud == s
        assert c.fecha_hora_real == _hora(9, 30)
...
class TestInmutabilidadComprobante:
    ...
    def _comprobante(self):
        return Comprobante(1, _solicitud_basica(), _hora(9, 30), "Juan")
